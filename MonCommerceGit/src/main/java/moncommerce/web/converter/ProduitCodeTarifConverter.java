...
public class ProduitCodeTarifConverter implements Converter {
    ...
    moncommerce.domain.model.ProduitCodeTarifPK getKey(String value) {
        moncommerce.domain.model.ProduitCodeTarifPK key;
        String values[] = value.split(SEPARATOR_ESCAPED);
        key = new moncommerce.domain.model.ProduitCodeTarifPK();
        key.setIDCodetarif(Long.parseLong(values[0]));
        key.setIDProduit(Long.parseLong(values[1]));
        return key;
    }

    String getStringKey(moncommerce.domain.model.ProduitCodeTarifPK value) {
