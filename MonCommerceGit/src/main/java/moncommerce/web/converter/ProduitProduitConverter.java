...
public class ProduitProduitConverter implements Converter {
    ...
    moncommerce.domain.model.ProduitProduitPK getKey(String value) {
        moncommerce.domain.model.ProduitProduitPK key;
        String values[] = value.split(SEPARATOR_ESCAPED);
        key = new moncommerce.domain.model.ProduitProduitPK();
        key.setIDProduitComposant(Long.parseLong(values[0]));
        key.setIDProduitComposee(Long.parseLong(values[1]));
        return key;
    }

    String getStringKey(moncommerce.domain.model.ProduitProduitPK value) {
