...
public class FactureClientProduitConverter implements Converter {
    ...
    moncommerce.domain.model.FactureClientProduitPK getKey(String value) {
        moncommerce.domain.model.FactureClientProduitPK key;
        String values[] = value.split(SEPARATOR_ESCAPED);
        key = new moncommerce.domain.model.FactureClientProduitPK();
        key.setIDProduit(Long.parseLong(values[0]));
        key.setIDFactureClient(Long.parseLong(values[1]));
        return key;
    }

    String getStringKey(moncommerce.domain.model.FactureClientProduitPK value) {
