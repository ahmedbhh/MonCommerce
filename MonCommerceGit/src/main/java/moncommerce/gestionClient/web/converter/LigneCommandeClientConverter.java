...
public class LigneCommandeClientConverter implements Converter {
    ...
    moncommerce.gestionClient.domain.model.LigneCommandeClientPK getKey(String value) {
        moncommerce.gestionClient.domain.model.LigneCommandeClientPK key;
        String values[] = value.split(SEPARATOR_ESCAPED);
        key = new moncommerce.gestionClient.domain.model.LigneCommandeClientPK();
        key.setIDCommandeClient(Long.parseLong(values[0]));
        key.setIDProduit(Long.parseLong(values[1]));
        return key;
    }

    String getStringKey(moncommerce.gestionClient.domain.model.LigneCommandeClientPK value) {
